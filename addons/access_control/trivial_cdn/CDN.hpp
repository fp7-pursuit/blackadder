...
class CDN:public CommChannel{
	public:
		 ...
		 */
		 CDN(std::string payloadRVSId,bool user_space, char bastrategy);
	...
	private:
		...
		/**
		 * a structure that is used to store publishers, ISP ID pairs
		 */
		 map<std::string,std::string> pubisp;
		 /**
		 * a structure that is used to store publication IDs, publishers
		 */
		 map<std::string,vector<std::string> > pubpub;
